...
echo ""
echo "Enter the number of the target disk (e.g. 1):"
read -r disk_num

TARGET_DISK=$(lsblk -dno NAME,TYPE | awk '$2=="disk"{print "/dev/"$1}' | sed -n "${disk_num}p")
...
if [ ! -b "$ROOT_PART" ]; then
    echo "ERROR: No ext4 partition found on $TARGET_DISK"
    echo "Make sure you created and formatted an ext4 partition."
    lsblk "$TARGET_DISK"
    exit 1
fi
...
# =============================================================================
# COPY SYSTEM - NO EXTRA CONFIRMATION
# =============================================================================
echo ""
...
# =============================================================================
# PENTEST TOOLS SELECTION (BY CATEGORY)
# =============================================================================
echo ""
    ...
    for cat in "\${SELECTED_CATEGORIES[@]}"; do
        case \$cat in
            ...
            4)
                ...
                git clone https://github.com/fortra/impacket.git || true
                ;;
        esac
    done
...
EOT
    ...
    #chmod +x /usr/share/applications/leakos-pentest-tools.desktop

    echo "Tools from selected categories cloned to /opt/pentest-tools"
    echo ".desktop entry created: LeakOS Pentest Tools (opens bash in the folder)"
else
